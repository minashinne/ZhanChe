...
//CAN设置
int txMsgID = 116;  //发送报文附带的ID
int rxMsgID = 116;  //准许接收指定附带ID的报文（过滤器控制，未进入loop模式，不会接收自己的报文）
eXoCAN can(STD_ID_LEN, BR250K, PORTA_11_12_XCVR);

//霍尔设置
int16_t Target_Offset_Angle_10 = 0;  //10倍目标角度偏移值000-600
float Target_Offset_Angle = 0;       //角度偏移值0.0-60.0
float Target_Angle = 10;             //目标角度0.0-60.0
float Current_Angle = 0;             //当前角度0-60
float Raw_Angle = 0;                 //读取的原始角度数值（0.00——60.00）
AS5600 as5600;
