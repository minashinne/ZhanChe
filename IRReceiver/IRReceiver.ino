...
// LED列刷新col为列数（0-7）,row为行数（8位位图）。
int col_rec = 0;
int row_rec = 0;
uint32_t color_rec = 0;
uint64_t pos_rec = 0;
void Led_Col(uint8_t col, uint8_t row, uint32_t color)
{
  for (int i = 0; i < 6; i++)
  {
    if (row & (1ULL << i))
    {                                            // 判断第 i 位是否为 1
      strip_l.setPixelColor(col * 6 + i, color); // 开灯，设置白色
      strip_r.setPixelColor(col * 6 + i, color); // 开灯，设置白色
    }
  }
  strip_l.show();
  strip_r.show();
...
// LED全亮，pos为所选LED位置（48位位图）。
void Led_All(uint64_t pos, uint32_t color)
{
  for (int i = 0; i < 48; i++)
  {
    if (pos & (1ULL << i))
    {                                  // 判断第 i 位是否为 1
      strip_l.setPixelColor(i, color); // 开灯，设置白色
      strip_r.setPixelColor(i, color); // 开灯，设置白色
    }
  }
  strip_l.show();
  strip_r.show();
...
void docmd(String cmdl)
{
  ...
  {
    if (sscanf(cmdl.c_str(), "COLOR %x", &color_rec) == 1)
    {
      Led_Back(color_rec);
    }
    else
  ...
  else if (cmdl.length() >= 6 && cmdl.substring(0, 6) == "LEDCol")
  {
    if (sscanf(cmdl.c_str(), "LEDCol %d %d %x", &col_rec, &row_rec, &color_rec) == 3)
    {
      Led_Col(col_rec, row_rec, color_rec);
    }
  }
  else if (cmdl.length() >= 6 && cmdl.substring(0, 6) == "LEDAll")
  {
    if (sscanf(cmdl.c_str(), "LEDAll %llu %x", &pos_rec, &color_rec) == 2)
    {
      Led_All(pos_rec, color_rec);
    }
  }
